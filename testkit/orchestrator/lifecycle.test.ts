...
test("E11: an unranked agent cannot register as orchestrator (event + storage)", async () => {
  ...
  assert.equal(r.error, "orchestrator.AgentMustBeRank2");
  assert.equal(findEvent(r.events, "orchestrator", "OrchestratorRegistered"), undefined);
  assert.equal(((await api.query.orchestrator.orchestratorRegistration(newbie.address)) as any).isNone, true);
});
...
test("self-link is rejected with no proposal stored", async (t) => {
  if (!(await ensureOrchestrator())) return t.skip("Alice cannot reach Rank 2 on this node");
  const r = await trySend(api, api.tx.orchestrator.proposeSubAgentLink(acc.Alice.address), acc.Alice);
  assert.equal(r.error, "orchestrator.SelfLink");
  assert.equal(findEvent(r.events, "orchestrator", "LinkProposed"), undefined);
  const p = (await api.query.orchestrator.pendingLinkProposals(acc.Alice.address, acc.Alice.address)) as any;
  assert.equal(p.isNone, true);
  assert.equal(((await linkOf(acc.Alice)) as any).isNone, true);
});

test("lifecycle: propose → accept → remove (events + both link maps + count)", async (t) => {
  if (!(await ensureOrchestrator())) return t.skip("Alice cannot reach Rank 2 on this node");
  ...
  assert.equal(proposed!.data[0].toString(), acc.Alice.address);
  assert.equal(proposed!.data[1].toString(), sub.address);
  assert.equal(((await api.query.orchestrator.pendingLinkProposals(acc.Alice.address, sub.address)) as any).isSome, true);
  ...
  assert.equal(((await linkOf(sub)) as any).unwrap().toString(), acc.Alice.address);
  assert.equal(((await api.query.orchestrator.subAgentLinks(acc.Alice.address, sub.address)) as any).isSome, true);
  assert.equal(((await api.query.orchestrator.pendingLinkProposals(acc.Alice.address, sub.address)) as any).isNone, true);
  assert.equal(await activeCount(acc.Alice), before + 1);
  ...
  assert.equal(((await linkOf(sub)) as any).isSome, true);
  ...
  assert.equal(removed!.data[2].toString(), acc.Alice.address, "`by` names the caller");
  assert.equal(((await linkOf(sub)) as any).isNone, true);
  assert.equal(((await api.query.orchestrator.subAgentLinks(acc.Alice.address, sub.address)) as any).isNone, true);
  assert.equal(await activeCount(acc.Alice), before);
});
