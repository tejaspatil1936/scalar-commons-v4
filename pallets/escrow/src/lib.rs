...
pub mod pallet {
    ...
    /// Lifetime count of agreements settled to completion (confirm or release paths).
    #[pallet::storage]
    pub type CompletedAgreements<T: Config> = StorageValue<_, u32, ValueQuery>;
        ...
        <T as agents_pallet::Config>::Currency: ReservableCurrency<T::AccountId>,
    {
        ...
        pub fn confirm_delivery(
            ...
            Agreements::<T>::try_mutate(&buyer, &provider, |vec| {
                ...
                CompletedAgreements::<T>::mutate(|c| *c = c.saturating_add(1));
                vec.swap_remove(idx);
                Ok::<(), DispatchError>(())
            })?;
