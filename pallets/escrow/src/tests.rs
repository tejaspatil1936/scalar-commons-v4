...
#[test]
fn completed_agreements_increments_on_confirm() {
    new_test_ext().execute_with(|| {
        register_both();
        create(1_000);
        frame_system::Pallet::<Test>::set_block_number(10);
        ...
        assert_eq!(CompletedAgreements::<Test>::get(), 1);
    });
}

#[test]
fn completed_agreements_increments_on_release() {
    new_test_ext().execute_with(|| {
        register_both();
        create(1_000);
        // Advance past deliver_by(500) + BuyerResponseWindow(50) to allow claim_refund
        frame_system::Pallet::<Test>::set_block_number(560);
        assert_ok!(Escrow::claim_refund(RuntimeOrigin::signed(ALICE), BOB, 0));
        assert_eq!(CompletedAgreements::<Test>::get(), 1);
    });
}
