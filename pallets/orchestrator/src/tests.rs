...
fn propose_and_accept_link_works() {
    new_test_ext().execute_with(|| {
        ...
        OrchestratorRegistration::<Test>::insert(ALICE, OrchestratorRecord {
            max_sub_agents: 10,
            fee_bps: 200,
            registered_at: 0,
            active_sub_count: 0,
        ...
        // Propose link
        assert_ok!(Orchestrator::propose_sub_agent_link(RuntimeOrigin::signed(ALICE), BOB));
        assert!(PendingLinkProposals::<Test>::contains_key(ALICE, BOB));

        // Accept link
        assert_ok!(Orchestrator::accept_orchestrator_link(RuntimeOrigin::signed(BOB), ALICE));
        ...
        assert_eq!(OrchestratorRegistration::<Test>::get(ALICE).unwrap().active_sub_count, 1);
    });
}
...
fn sub_agent_cannot_link_to_two_orchestrators() {
    new_test_ext().execute_with(|| {
        OrchestratorRegistration::<Test>::insert(ALICE, OrchestratorRecord {
            max_sub_agents: 10, fee_bps: 200, registered_at: 0, active_sub_count: 0,
        });
        OrchestratorRegistration::<Test>::insert(CAROL, OrchestratorRecord {
            max_sub_agents: 10, fee_bps: 100, registered_at: 0, active_sub_count: 0,
        });
        register(BOB, 1_000);
        ...
        // Propose from CAROL
        assert_ok!(Orchestrator::propose_sub_agent_link(RuntimeOrigin::signed(CAROL), BOB));
...
fn remove_link_by_sub_agent_works() {
    new_test_ext().execute_with(|| {
        OrchestratorRegistration::<Test>::insert(ALICE, OrchestratorRecord {
            max_sub_agents: 10, fee_bps: 200, registered_at: 0, active_sub_count: 1,
        });
        register(BOB, 1_000);
        SubAgentLinks::<Test>::insert(ALICE, BOB, SubLinkRecord { linked_at: 0, lifetime_volume: 0 });
        SubAgentToOrchestrator::<Test>::insert(BOB, ALICE);

        // BOB removes the link (passing orchestrator as the 'other' party)
        assert_ok!(Orchestrator::remove_sub_agent_link(RuntimeOrigin::signed(BOB), ALICE));
        assert!(!SubAgentLinks::<Test>::contains_key(ALICE, BOB));
        assert!(SubAgentToOrchestrator::<Test>::get(BOB).is_none());
        assert_eq!(OrchestratorRegistration::<Test>::get(ALICE).unwrap().active_sub_count, 0);
    });
}
...
fn deregister_clears_all_links() {
    new_test_ext().execute_with(|| {
        OrchestratorRegistration::<Test>::insert(ALICE, OrchestratorRecord {
            max_sub_agents: 10, fee_bps: 200, registered_at: 0, active_sub_count: 2,
        });
        register(BOB, 1_000);
        register(CAROL, 1_000);
        SubAgentLinks::<Test>::insert(ALICE, BOB, SubLinkRecord { linked_at: 0, lifetime_volume: 0 });
        SubAgentLinks::<Test>::insert(ALICE, CAROL, SubLinkRecord { linked_at: 0, lifetime_volume: 0 });
        SubAgentToOrchestrator::<Test>::insert(BOB, ALICE);
        SubAgentToOrchestrator::<Test>::insert(CAROL, ALICE);

        assert_ok!(Orchestrator::deregister_orchestrator(RuntimeOrigin::signed(ALICE)));
