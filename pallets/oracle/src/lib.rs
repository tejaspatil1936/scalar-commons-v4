...
pub mod pallet {
        ...
        BalanceOf<T>: From<u32>,
    {
        ...
        pub fn finalise_request(origin: OriginFor<T>, request_id: [u8; 32]) -> DispatchResult {
            ...
            ensure!(req.response_count >= req.min_responses, Error::<T>::InsufficientResponses);
            ...
            if winner_count > 0 && consensus_reached {
                let share = req.bounty.checked_div(&winner_count.into()).unwrap_or_default();
                for winner in &winners {
                    let _ = <T as agents_pallet::Config>::Currency::repatriate_reserved(
                        &req.creator, winner, share,
                        frame_support::traits::tokens::BalanceStatus::Free,
                    );
                    Self::update_accuracy(winner, capability, true);
                }
            }
            for loser in &losers { Self::update_accuracy(loser, capability, false); }
            ...
            if let Some(ctx) = &req.dispute_context {
                let provider_wins_hash = sp_io::hashing::blake2_256(
                    crate::escrow_bridge::PROVIDER_WINS_PREIMAGE
                );
                let provider_wins = winning_hash == provider_wins_hash;
                let _ = T::DisputeCallback::on_dispute_resolved(
                    &ctx.buyer, &ctx.provider, ctx.seq, provider_wins,
                );
            }
            ...
            Self::deposit_event(Event::OracleRequestFinalised {
                id: request_id, winning_hash, respondents_paid: winner_count,
            });
            Ok(())
        }
        ...
        pub fn expire_request(origin: OriginFor<T>, request_id: [u8; 32]) -> DispatchResult {
            ...
            let req = OracleRequests::<T>::get(request_id).ok_or(Error::<T>::RequestNotFound)?;
                ...
                || (req.status == OracleRequestStatus::Collecting && req.response_count < req.min_responses),
                Error::<T>::RequestAlreadyFinalised
            );
            let now = frame_system::Pallet::<T>::block_number();
        ...
        pub fn batch_submit_response(
            ...
            for (request_id, answer_hash, _capability) in submissions.iter() {
                let result: Result<(), DispatchError> = OracleRequests::<T>::try_mutate(
                    *request_id, |maybe_req| -> DispatchResult {
                        let req = maybe_req.as_mut().ok_or(Error::<T>::RequestNotFound)?;
                        ensure!(
                            req.status == OracleRequestStatus::Open || req.status == OracleRequestStatus::Collecting,
                            Error::<T>::RequestAlreadyFinalised
            ...
            Self::deposit_event(Event::BatchResponseSubmitted { agent, accepted, skipped });
            Ok(())
        }
    }
        ...
        BalanceOf<T>: From<u32>,
    {
        pub fn best_score(who: &T::AccountId) -> u32 {
            OracleScore::<T>::iter_prefix(who).map(|(_, score)| score).max().unwrap_or(0)
        }

        pub fn post_dispute_question_internal(
            buyer: &T::AccountId, provider: &T::AccountId, seq: u32,
            bounty: BalanceOf<T>, deadline: BlockNumberFor<T>, capability: Option<u32>,
            ...
            OracleRequests::<T>::insert(question_hash, OracleRequest::<T> {
                creator: buyer.clone(), question_hash, bounty,
                mode: ConsensusMode::Factual, min_responses: 3,
                consensus_threshold: 67, response_deadline: deadline,
                challenge_window: T::MinChallengeWindow::get(),
                required_capability: capability, status: OracleRequestStatus::Open,
                response_count: 0,
                dispute_context: Some(DisputeContext { buyer: buyer.clone(), provider: provider.clone(), seq }),
            });
            EraTotalQuestions::<T>::mutate(|c| *c = c.saturating_add(1));
            Ok(question_hash)
        }
        ...
        fn update_accuracy(who: &T::AccountId, capability: u32, correct: bool) {
            OracleAccuracy::<T>::mutate(who, capability, |(c, t)| {
                if correct { *c = c.saturating_add(1); }
                *t = t.saturating_add(1);
            });
            let (correct_count, total) = OracleAccuracy::<T>::get(who, capability);
            let score = (correct_count as u64).saturating_mul(10_000)
                .checked_div(total.max(1) as u64).unwrap_or(0) as u32;
            OracleScore::<T>::insert(who, capability, score);
            Self::deposit_event(Event::ScoreUpdated { agent: who.clone(), capability, new_score: score });
        }
    }
    ...
    where
        BalanceOf<T>: From<u32>,
    {
        fn best_score(who: &T::AccountId) -> u32 { Pallet::<T>::best_score(who) }
    }

    impl<T: Config> crate::escrow_bridge::DisputeOracle<T::AccountId, BalanceOf<T>, BlockNumberFor<T>>
        for Pallet<T>
    where
        BalanceOf<T>: From<u32>,
    {
        fn post_dispute_question(
            ...
            Pallet::<T>::post_dispute_question_internal(buyer, provider, seq, bounty, deadline, capability)
        }
    }
}
