...
// ── H-0d boundary tests: QuorumNotMet threshold arithmetic ──────────────────

#[test]
fn quorum_zero_voters() {
    // compute_factual_consensus with an empty slice must return no consensus
    // and produce no winners or losers (no panic, no division by zero).
    new_test_ext().execute_with(|| {
        let responses: Vec<(u64, [u8; 32])> = vec![];
        let (hash, winners, losers) = Oracle::compute_factual_consensus_test(&responses, 67);
        assert!(hash.is_none());
...
fn quorum_exactly_met() {
    // 3 voters, threshold=67 → required = (3*67)/100 = 2.
    // Exactly 2 voters agree → consensus reached.
    new_test_ext().execute_with(|| {
        ...
        let (hash, winners, losers) = Oracle::compute_factual_consensus_test(&responses, 67);
        assert_eq!(hash, Some(agree_hash));
...
#[test]
fn quorum_one_below() {
    // 3 voters, threshold=67 → required = 2.
    // Only 1 voter agrees (quorum-minus-1) → no consensus.
    new_test_ext().execute_with(|| {
        let responses: Vec<(u64, [u8; 32])> =
