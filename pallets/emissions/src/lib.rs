        ...
        u128: TryInto<BalanceOf<T>>,
    {
        ...
        fn compute_weight_cached(
            ...
            let work_score = if raw_vol == 0 || diversity_bps == 0 {
                ...
                raw_vol.saturating_mul(diversity_bps) / SCORE_SCALE.max(1)
            };
            ...
            let gov_score = gov_votes.min(max_props).saturating_mul(SCORE_SCALE) / max_props.max(1);
            ...
            let gov_contribution = if work_score > 0 {
                alpha.saturating_mul(gov_score) / SCORE_SCALE.max(1)
            } else {
                0
            };
            ...
            let activity = (effective_floor
                ...
                .saturating_add(beta.saturating_mul(work_score) / SCORE_SCALE.max(1)))
            .min(BPS_SCALE);
