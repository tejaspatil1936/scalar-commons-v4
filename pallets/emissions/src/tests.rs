...
#[test]
fn settle_era_zero_agents_no_panic() {
    new_test_ext().execute_with(|| {
        // No agents registered. settle_era must not panic and must record floor emission.
        let era_dur = <Test as crate::pallet::Config>::EraDuration::get();
        System::set_block_number(era_dur);
        assert_ok!(Emissions::settle_era(RuntimeOrigin::signed(99)));
        assert_eq!(
            LastEraEmission::<Test>::get(),
            FloorEmission::get(),
            "zero-agent era should emit the floor amount"
        );
    });
}
