...
pub mod pallet {
    ...
    /// The () default returns `true` for is_actively_voting so existing unit tests
    /// pass without a mock. verify() uses real sp_io::crypto::sr25519_verify.
    pub trait GovVoteVerifier<AccountId> {
        fn is_actively_voting(who: &AccountId) -> bool;
        /// Verifies that `sig` is a valid sr25519 signature over `msg` by `pk`.
        /// Default implementation calls sp_io::crypto::sr25519_verify (real host function).
        /// All impls inherit this — override only in tests that need a mock.
        fn verify(sig: &[u8; 64], msg: &[u8], pk: &[u8; 32]) -> bool {
            let signature = sp_core::sr25519::Signature::from_raw(*sig);
            let public = sp_core::sr25519::Public::from_raw(*pk);
            sp_io::crypto::sr25519_verify(&signature, msg, &public)
        }
    }
    impl<AccountId> GovVoteVerifier<AccountId> for () {
        ...
        // verify() uses the trait default (real sp_io::crypto::sr25519_verify)
    }
