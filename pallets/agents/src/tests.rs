...
// ── H-0b: GovVoteVerifier real sr25519 signature verification ─────────────────

#[test]
fn gov_vote_verifier_valid_sig_accepted() {
    new_test_ext().execute_with(|| {
        use sp_core::{sr25519, Pair};
        let (pair, _) = sr25519::Pair::generate();
        let msg = b"sc:gov-vote:1";
        let sig = pair.sign(msg);
        let pk = pair.public();
        let sig_bytes: [u8; 64] = sig.as_ref().try_into().expect("sig is 64 bytes");
        let pk_bytes: [u8; 32] = pk.as_ref().try_into().expect("pk is 32 bytes");
        assert!(<() as GovVoteVerifier<u64>>::verify(
            &sig_bytes, msg, &pk_bytes
        ));
    });
}

#[test]
fn gov_vote_verifier_invalid_sig_rejected() {
    new_test_ext().execute_with(|| {
        use sp_core::{sr25519, Pair};
        let (pair, _) = sr25519::Pair::generate();
        let msg = b"sc:gov-vote:1";
        let sig = pair.sign(msg);
        let pk = pair.public();
        let mut bad_sig: [u8; 64] = sig.as_ref().try_into().expect("sig is 64 bytes");
        let pk_bytes: [u8; 32] = pk.as_ref().try_into().expect("pk is 32 bytes");
        bad_sig[0] ^= 0xff; // corrupt first byte — signature must not verify
        assert!(!<() as GovVoteVerifier<u64>>::verify(
            &bad_sig, msg, &pk_bytes
        ));
    });
}
