...
fn rank_promotion_chain_zero_to_three() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(final_completions, 3, "Should have 3 completions for Rank 3 eligibility");
...
fn span_gate_blocks_early_rank3() {
    new_test_ext().execute_with(|| {
        ...
        assert!(now >= registered_at, "Span gate condition should pass in test");
    });
}
...
fn oracle_score_gate_for_rank3() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(min_score, 0, "Oracle gate should be disabled in test config");
    });
}
