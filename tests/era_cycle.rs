...
fn era_cycle_full() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(alice_debt, acc_before, "Debt should equal acc at registration time");
        ...
        assert_eq!(alice_vol_post, 0, "EraEscrowVolume should be cleared after drain");

        // ── Claim emissions ───────────────────────────────────────────────────
        let alice_bal_before = pallet_balances::Pallet::<TestRuntime>::free_balance(ALICE);
        assert_ok!(pallet_emissions::Pallet::<TestRuntime>::claim(RuntimeOrigin::signed(ALICE)));
        let alice_bal_after = pallet_balances::Pallet::<TestRuntime>::free_balance(ALICE);
        assert!(alice_bal_after > alice_bal_before, "Alice should receive emissions");
        ...
        let carol_claim = pallet_emissions::Pallet::<TestRuntime>::claim(RuntimeOrigin::signed(CAROL));
        assert!(carol_claim.is_err(), "Carol (no provider volume) should have nothing to claim");
