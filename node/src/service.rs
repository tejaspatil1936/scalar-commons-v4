//! Service implementation — the wiring that turns the runtime WASM blob into a
//! block-producing node.
//!
//! Ported from `substrate/bin/node/cli/src/service.rs` at tag
//! `polkadot-stable2503`. That node is the reference because its consensus
//! shape is ours: BABE authoring, GRANDPA finality, Session + Historical for
//! key ownership proofs, Staking for the validator set.
//!
//! Four subsystems the reference wires are **omitted**, not stubbed, because
//! this runtime does not contain them. Each omission removes a block-import
//! layer or a spawned task; none of them fakes a result:
//!
//! * **BEEFY** (`sc-consensus-beefy`) — needs `pallet-beefy` + `pallet-mmr` and
//!   the `BeefyApi` runtime API. Consequence: the block-import chain here is
//!   `grandpa -> babe` where the reference is `grandpa -> beefy -> babe`.
//! * **MMR gadget** (`mmr-gadget`) — same reason; it only exists to serve BEEFY.
//! * **Statement store** (`sc-statement-store`) — needs
//!   `pallet-statement-store` and `sp_statement_store::runtime_api`. Its
//!   absence is also why `HostFunctions` below is just
...
//! pulls in `frame-benchmarking-cli`, whose pallet-benchmark path needs a
//! `runtime-benchmarks` feature this runtime only partially wires. The
//! `--no-hardware-benchmarks` flag is kept so scripts that pass it do not break.
...
use scalar_commons_runtime::{opaque::Block, RuntimeApi};
use sp_runtime::traits::Block as BlockT;
use std::{path::Path, sync::Arc};

use crate::cli::Cli;

/// Host functions required by the Scalar Commons runtime.
///
/// The reference adds `sp_statement_store::runtime_api::HostFunctions` here;
/// this runtime has no statement store, so the standard Substrate set is the
/// whole of it.
pub type HostFunctions = sp_io::SubstrateHostFunctions;

/// A specialized `WasmExecutor` providing all required host functions.
...
/// The minimum period of blocks on which justifications will be imported and
/// generated.
...
> {
    ...
    // Reference chains grandpa -> beefy -> babe. No BEEFY here, so BABE imports
    // straight on top of GRANDPA.
...
pub fn new_full_base<N: NetworkBackend<Block, <Block as BlockT>::Hash>>(
    ...
    if let sc_service::config::Role::Authority { .. } = &role {
        ...
        let babe_config = sc_consensus_babe::BabeParams {
            ...
            // The reference appends a third inherent here,
            // `sp_transaction_storage_proof::registration::new_data_provider`,
            // which exists to feed `pallet-transaction-storage`. This runtime
            // has no such pallet, so supplying that inherent would produce a
            // block the runtime rejects as containing an unknown inherent.
